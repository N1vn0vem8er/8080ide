...
private:
    ...
    static inline std::unordered_map<std::string_view, unsigned char> oneargs{ {"MVI A,", 0x3e} , {"MVI B,", 0x06},{"MVI D,",0x16},{"MVI H,",0x26},
    ...
    static inline std::unordered_map<std::string_view, unsigned char> twoargs{ {"LXI B,",0x01},{"LXI D,",0x11},{"LXI H,",0x21},{"LXI SP,",0x31},{"SHLD ",0x22},{"STA ",0x32},
                                                              {"LHLD ",0x2a}, {"LDA ", 0x3a} };
    static inline std::unordered_map<std::string_view, unsigned char> callsandjmps{ {"CNZ ",0xc4},{"CNC ",0xd4},
    ...
    static inline std::set<std::string_view> noargsMnemonics{
    ...
    static inline std::set<std::string_view> oneargsMnemonics{
        "MVI", "OUT", "IN", "ADI", "SUI", "ANI", "ORI", "ACI", "SBI", "XRI",
        "CPI"
    };

    static inline std::set<std::string_view> twoargsMnemonics{
        "LXI", "SHLD", "STA", "LHLD", "LDA"
    };

    static inline std::set<std::string_view> callsandjmpsMnemonics{
        "CNZ", "CNC", "CPO", "CP", "CZ", "CC", "CPE", "CM", "CALL", "JNZ",
        "JNC", "JPO", "JP", "JMP", "JZ", "JC", "JPE", "JM"
    };

public:
    static inline std::unordered_map<std::string, std::vector<std::string>> buildInMacros{
        {"PRINTCHAR", {"OUT 01\n"}},
        {"INCHAR", {"IN 00\n"}},
        {"GCLEAR", {"PUSH PSW\n", "MVI A,00\n", "OUT 05\n", "POP PSW\n"}},
        {"GDRAW", {"OUT 04\n"}},
